...
module Ifetc32(Instruction,Add_result,Read_data_1,next_PC,PC_plus_4_out,Branch,nBranch,Jmp,Jal,Jrn,Zero,clock,reset,opcplus4,PC,PC15_2);
   ...
   //分配64KB ROM，编译器实际只用 64KB ROM
    ...
    always @* begin  // beq $n ,$m if $n=$m branch   bne if $n /=$m branch jr
        if (Branch && Zero) next_PC = Add_result >> 2;
        else if (Branch && !Zero) next_PC = PC_plus_4_out >> 2;
        else if (nBranch && Zero) next_PC = PC_plus_4_out >> 2;
        else if (nBranch && !Zero) next_PC = Add_result >> 2;
        if (Jrn) next_PC = Read_data_1;
    end
    
    always @(negedge clock) begin  //（含J，Jal指令和reset的处理）
        if (reset) begin PC = 0; next_PC = 1; end
        else if (Jmp) PC = {{PC_plus_4_out[31:28]}, {Instruction[25:0]}, {2'b00}};
        else if (Jal) begin
            opcplus4 = next_PC;
            PC = {{PC_plus_4_out[31:28]}, {Instruction[25:0]}, {2'b00}};
        end else PC = next_PC << 2;
    end
endmodule